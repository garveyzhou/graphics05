...
void parse_file ( char * filename,
  ...
  while ( fgets(line, sizeof(line), f) != NULL ) {
    ...
    if ( strncmp(line, "line", strlen(line)) == 0 ) {
      ...
      add_curve(edges, xvals[0], yvals[0], xvals[1], yvals[1], xvals[2], yvals[2], xvals[3], yvals[3],0.005,BEZIER);
    }
    else if( strncmp(line,"hermite",strlen(line)) == 0){
      fgets(line, 255, f);
      sscanf(line, "%lf %lf %lf %lf %lf %lf %lf %lf",xvals, yvals, xvals+1, yvals+1, xvals+2, yvals+2, xvals+3, yvals+3);
      add_curve(edges, xvals[0], yvals[0], xvals[1], yvals[1], xvals[2], yvals[2], xvals[3], yvals[3],0.005,HERMITE);
    }
    else if (strncmp(line,"circle",strlen(line)) == 0){
      fgets(line,255,f);
      sscanf(line,"%lf %lf %lf %lf",xvals,yvals,zvals,&radius);
      add_circle(edges,xvals[0],yvals[0],zvals[0],radius,0.005);
    }
